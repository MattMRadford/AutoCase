 ...
 -> X  - '${str}'  : (${form})  `);
  ...
  function trackUsageAndPromptReview() {
    ...
    if (usageTracker.count === 3 && !usageTracker.reviewRequested) {
      setTimeout(() => showReviewPrompt(), 1e3);
  ...
  `;
    ...
    const extensionId = "inmghklhnephcaocldmdbikmipomhjnd";
        ...
        if (doc && win) {
          doc.addEventListener("mouseup", () => {
            chrome.storage.sync.get(["autocaseEnabled", "autocaseMode"], (data) => {
              if (!data.autocaseEnabled) return;
              const mode2 = data.autocaseMode || "title";
              let obj2 = getSelectedText().then(applyCaseChange);
              ...
              if (selected) {
                if (isEdit) {
                  ...
                  let obj3 = {
                    type: "iframe-edit",
                    element: isEdit,
                    text: applyCasing(selected, mode2)
                  };
                  applyCaseChange(obj3);
                } else {
                  let obj3 = {
                    type: "iframe-nonedit",
                    element: iframe,
  ...
  function smartTitleCase(text) {
    ...
    const allTerms = doc.terms().json().flatMap((d2) => d2.terms);
    return allTerms.map((term, i3, arr) => {
      const word = term.text;
      const tag = term.tags.join(",");
      const isFirstOrLast = i3 === 0 || i3 === arr.length - 1;
  ...
  function toSentenceCase(text) {
    return text.match(/[^.!?]+[.!?]*\s*/g).map((sentence) => {
  ...
  function toSnakeCase(text) {
    return text.trim().toLowerCase().replace(/[^\w\s]/g, "").split(/\s+/).join("_");
  }
  function toCamelCase(text) {
    const words = text.trim().toLowerCase().replace(/[^\w\s]/g, "").split(/\s+/);
  ...
  async function getSelectedText() {
    ...
    const editable = shadowRoot?.activeElement?.closest('[contenteditable="true"]') || shadowRoot?.querySelector('[contenteditable="true"]');
  ...
  function applyCaseChange(object) {
    ...
    if (type == "input-textarea" || type == "shadow-input") {
      if (element.selectionStart != null) {
        ...
        const before2 = element.value.slice(0, start2);
        const after2 = element.value.slice(end2);
        element.value = before2 + text + after2;
        element.setSelectionRange(start2, start2 + text.length);
      }
